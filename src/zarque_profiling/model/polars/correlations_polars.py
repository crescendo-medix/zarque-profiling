...
from zarque_profiling.model.polars.discretize_polars import (
    DiscretizationType,
    Discretizer,
)

# TODO: Correlation functions are not well supported in Polars, 
# so use Pandas functions before they are supported in Polars.


@Spearman.compute.register(Settings, pol.DataFrame, dict)
def polars_spearman_compute(
    config: Settings, df: pol.DataFrame, summary: dict
) -> Optional[pd.DataFrame]:
    
    df = df.to_pandas()   # TODO: Convert to Pandas DF to use Pandas correlation functions.
    
    return df.corr(method="spearman")


@Pearson.compute.register(Settings, pol.DataFrame, dict)
def polars_pearson_compute(
    config: Settings, df: pol.DataFrame, summary: dict
) -> Optional[pd.DataFrame]:
...
@Kendall.compute.register(Settings, pol.DataFrame, dict)
def polars_kendall_compute(
    config: Settings, df: pol.DataFrame, summary: dict
) -> Optional[pd.DataFrame]:
...
@Cramers.compute.register(Settings, pol.DataFrame, dict)
def polars_cramers_compute(
    config: Settings, df: pol.DataFrame, summary: dict
) -> Optional[pd.DataFrame]:
...
@PhiK.compute.register(Settings, pol.DataFrame, dict)
def polars_phik_compute(
    config: Settings, df: pol.DataFrame, summary: dict
) -> Optional[pd.DataFrame]:
...
@Auto.compute.register(Settings, pol.DataFrame, dict)
def polars_auto_compute(
    config: Settings, df: pol.DataFrame, summary: dict
) -> Optional[pd.DataFrame]:

...
@get_duplicates.register(Settings, pol.DataFrame, Sequence)
def polars_get_duplicates(
    config: Settings, df: pol.DataFrame, supported_columns: Sequence
) -> Tuple[Dict[str, Any], Optional[pol.DataFrame]]:
    ...
    if n_head > 0:
        if supported_columns and len(df) > 0:
            ...
            duplicated_rows = df.filter(df.is_duplicated()).to_pandas()
            duplicated_rows = (
                duplicated_rows

...
from zarque_profiling.model.polars.utils_polars import weighted_median
...
def get_character_counts(series: pol.Series) -> Counter:
    ...
    """
    series = pd.Series(series.get_columns()[0], index=series.get_columns()[1])
...
def unicode_summary_vc(vc: pol.Series) -> dict:
    vc = pd.Series(vc.get_columns()[0], index=vc.get_columns()[1])
    try:
    ...
    summary = {
        ...
        "n_characters": np.sum(character_counts_series),
...
def word_summary_vc(vc: pol.Series, stop_words: List[str] = []) -> dict:
    ...
    series = pd.Series(vc.get_columns()[0], index=vc.get_columns()[1])
...
def polars_describe_categorical_1d(
    ...
    series = series.cast(pol.Utf8)

    # Only run if at least 1 non-missing value
    value_counts = summary["value_counts_without_nan"]
    #value_counts.index = value_counts.index.astype(str)   # TODO: OK?
    ...
    if chi_squared_threshold > 0.0:
        summary["chi_squared"] = chi_square(histogram=value_counts.get_columns()[1])

...
def stationarity_test(config: Settings, series: pol.Series) -> Tuple[bool, float]:
    significance_threshold = config.vars.timeseries.significance

    # make sure the data has no missing values
    adfuller_test = adfuller(series.drop_nans())
...
def seasonality_test(series: pol.Series, mad_threshold: float = 6.0) -> Dict[str, Any]:
...
def get_fft(series: pol.Series) -> pol.DataFrame:
    """Computes FFT
    ...
    return pol.DataFrame({"freq": freq, "ampl": ampl})


def get_fft_peaks(
    fft: pol.DataFrame, mad_threshold: float = 6.0
) -> Tuple[float, pol.DataFrame, pol.DataFrame]:
...
@series_hashable
@series_handle_nulls
def polars_describe_timeseries_1d(

...
from zarque_profiling.utils.imghdr_patch import *  # noqa: F401,F403
...
def extract_exif_series(image_exifs: list) -> dict:
    ...
    series = {"exif_keys": pol.Series(exif_keys, dtype=object).value_counts().to_dict()}

    for k, v in exif_values.items():
        series[k] = pol.Series(v).value_counts()
...
def image_summary(series: pol.Series, exif: bool = False, hash: bool = False) -> dict:
    """
    ...
    summary = {
        ...
        "image_dimensions": pol.Series(
            [x["size"] for x in image_information if "size" in x],
            name="image_dimensions",
        ),
    }

    image_widths = summary["image_dimensions"].map_dict(lambda x: x[0])
    summary.update(named_aggregate_summary(image_widths, "width"))
    image_heights = summary["image_dimensions"].map_dict(lambda x: x[1])

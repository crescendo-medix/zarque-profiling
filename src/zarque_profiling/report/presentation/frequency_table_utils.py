...
def _frequency_table(
    ...
    if isinstance(freqtable, pol.DataFrame):
        
        # polars
        if max_number_to_print < len(freqtable):
            freq_other = freqtable.get_columns()[1][max_number_to_print:].sum()
            min_freq = freqtable.get_columns()[1][max_number_to_print]
        else:
            freq_other = 0
            min_freq = 0

        freq_missing = n - freqtable.get_columns()[1].sum()
        # No values
        if len(freqtable) == 0:
            return []

        max_freq = max(freqtable.get_columns()[1][0], freq_other, freq_missing)
        ...
        rows = []
        for row in freqtable[0:max_number_to_print].iter_rows():
            label = row[0]
            freq = row[1]
            rows.append(
                {
        ...
        if freq_other > min_freq:
            other_count = str(len(freqtable) - max_number_to_print)
            rows.append(
                {
...
def _extreme_obs_table(
    ...
    if isinstance(freqtable, pol.DataFrame):
        ...
        # polars
        obs_to_print = freqtable[:number_to_print]
        max_freq = obs_to_print.get_columns()[1].max()
        rows = []
        for row in obs_to_print.iter_rows():
            label = row[0]
            freq = row[1]
            rows.append(
                {

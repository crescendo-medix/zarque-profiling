...
from zarque_profiling.config import Correlation, Settings
...
def compare(
    ...
    for report in reports[1:]:
        ...
        if isinstance(report.df, pol.DataFrame):
            report.df = report.df[:, cols_2_compare]  # type: ignore
            reports = [r for r in reports if not r.df.is_empty()]  # type: ignore
        else:
    ...
    profile = ProfileReport(None, compare_mode = True, config = _config)    # TODO: compare_mode = True

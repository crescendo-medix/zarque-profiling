...
from polars import DataFrame as polDataFrame
...
from zarque_profiling.config import Config, Settings, PolarsSettings
from zarque_profiling.expectations_report import ExpectationsReport
from zarque_profiling.model.alerts import AlertType
from zarque_profiling.model.describe import describe as describe_df
from zarque_profiling.model.sample import Sample
...
from zarque_profiling.report import get_report_structure
from zarque_profiling.report.presentation.core import Root
from zarque_profiling.report.presentation.flavours.html.templates import (
    create_html_assets,
)
from zarque_profiling.serialize_report import SerializeReport
from zarque_profiling.utils.dataframe import hash_dataframe
from zarque_profiling.utils.paths import get_config


@typechecked
class ProfileReport(SerializeReport, ExpectationsReport):
    """Generate a profile report from a Dataset stored as a polars or pandas `DataFrame`.
    ...
    def __init__(
        self,
        df: Optional[Union[pd.DataFrame, polDataFrame]] = None,
        minimal: bool = True,
        tsmode: bool = False,
        ...
        **kwargs,
        ...
        """Generate a ProfileReport based on a polars or pandas DataFrame
        ...
        if compare_mode:    # TODO: Called in Compare Profile report mode
            pass
        else:
    ...
    def __validate_inputs(
        df: Union[pd.DataFrame, polDataFrame],
        minimal: bool,
        tsmode: bool,
        ...
        # Pandas Dataframe validations
        if isinstance(df, pd.DataFrame):
            if df is not None and df.empty:
                raise ValueError(
                    "DataFrame is empty. Please" "provide a non-empty DataFrame."
                )
        # Polars Dataframe validations
        elif isinstance(df, pol.DataFrame):
            if df is not None and df.is_empty():
                raise ValueError(
                    "DataFrame is empty. Please" "provide a non-empty DataFrame."
                )
            if tsmode:
                raise NotImplementedError(
                    "Time-Series dataset analysis is not yet supported for Polars DataFrames"
                )


    @staticmethod
    def __initialize_dataframe(
        df: Union[pd.DataFrame, polDataFrame], report_config: Settings
    ) -> Union[pd.DataFrame, polDataFrame]:
        if (
            df is not None
    ...
    def _render_html(self) -> str:
        from zarque_profiling.report.presentation.flavours import HTMLReport
        ...
        with tqdm(
            ...
            html = HTMLReport(copy.deepcopy(report)).render(
                ...
                version=self.description_set["package"]["zarque_profiling_version"],
            )
    ...
    def _render_widgets(self) -> Any:
        from zarque_profiling.report.presentation.flavours import WidgetReport
    ...
    def to_notebook_iframe(self) -> None:
        ...
        from zarque_profiling.report.presentation.flavours.widget.notebook import (
            get_notebook_iframe,
        )
    ...
    def compare(
        ...
        Alias for:
        ```
        zarque_profiling.compare([report1, report2], config=config)
        ```
        See `zarque_profiling.compare` for details.
